# main_texture.py
# Experiment 7 — Two uncorrelated spurious variables (color + texture)
#
# Extends Colored MNIST with a second spurious feature: stripe texture
# applied to background pixels. Color and texture are uncorrelated and 
# vary within the environments.
#
# Causal feature:     digit shape (invariant across environments)
# Spurious feature 1: color (red=0, green=1, corr=e with label)
# Spurious feature 2: background stripe pattern (corr=1-e with label)
#
...
# (shape) degrade when the two spurious correlations are uncorrelated?
...
ENV_CONFIGS = {
    "swap": {
        "color_e_values":   [0.1, 0.9],
        "texture_e_values": [0.9, 0.1],
        "description": "Swap — e1: color weak/texture strong, e2: color strong/texture weak"
    },
}
...
def make_environment_texture(images, labels, color_e, texture_e, device):
    """
    ...
    Color and texture have different correlation with label.
    ...
    color_label = torch.logical_xor(
        labels.bool(),
        torch.bernoulli(color_e * torch.ones(*labels.shape)).bool()
    ).float()
    ...
    texture_label = torch.logical_xor(
        labels.bool(),
        torch.bernoulli(texture_e * torch.ones(*labels.shape)).bool()
    ).float()
...
def get_args():
    ...
    parser.add_argument("--config", type=str, default="swap",
                        choices=["swap"])
...
def load_environments(config, selection_method, device, val_fraction=0.2):
    ...
    color_e_values   = cfg["color_e_values"]
    texture_e_values = cfg["texture_e_values"]
    ...
    split_size = n_total // len(color_e_values)

    train_envs = []
    val_envs   = []

    for i in range(len(color_e_values)):
        color_e   = color_e_values[i]
        texture_e = texture_e_values[i]
        start = i * split_size
        end   = start + split_size if i < len(color_e_values) - 1 else n_total
        ...
        if selection_method == "train_domain_val":
            ...
            train_envs.append(make_environment_texture(
                env_images[:n_train], env_labels[:n_train],
                color_e, texture_e, device))
            val_envs.append(make_environment_texture(
                env_images[n_train:], env_labels[n_train:],
                color_e, texture_e, device))
        else:
            train_envs.append(make_environment_texture(
                env_images, env_labels, color_e, texture_e, device))
            val_envs.append(make_environment_texture(
                env_images, env_labels, color_e, texture_e, device))

    # Test environment: both color and texture anti-correlated (e=0.9)
    test_env = make_environment_texture(
        test_pool[0], test_pool[1], TEST_E, TEST_E, device)
...
def main():
    ...
    print(f"Config: {flags.config}  |  "
          f"color_e: {cfg['color_e_values']}  |  "
          f"texture_e: {cfg['texture_e_values']}  |  "
