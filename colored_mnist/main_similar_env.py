# main_similar_env.py
# Tests the number of environments hypothesis while keeping e values
# within the SAME range {0.1, 0.2} across all conditions.
#
# Unlike main_env.py where adding environments also increased e diversity
# (potentially getting closer to test e=0.9), here all environments
# stay within {0.1, 0.2} — isolating the pure "number of environments" effect.
#
# ENV_E_VALUES:
#   n=1: {0.15}              ← midpoint
#   n=2: {0.1, 0.2}          ← original
#   n=3: {0.1, 0.15, 0.2}
#   n=4: {0.1, 0.13, 0.17, 0.2}
#   n=5: {0.1, 0.125, 0.15, 0.175, 0.2}
#
...
# ---------------------------------------------------------------------------
# Environment configuration — fixed e range {0.1, 0.2}
# ---------------------------------------------------------------------------
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Colored MNIST — Similar Environment Range IRM")
...
def load_environments(n_envs, selection_method, device, val_fraction=0.2):
    ...
    all within the fixed e range {0.1, 0.2}.
