# main_exp7.py — Experiment 7: Two Correlated Spurious Features
#
# Trains IRM and ERM on Colored MNIST with TWO spurious features:
#   1. Color: digit pixels red/green, correlated with label at rate e
#   2. Texture: background stripe pattern, correlated with label at rate e
#
# 4 configs matching Exp 5 proximity/diversity 2x2 design:
...
#
# Usage
#   python src/exp7/main_exp7.py --config A --mode irm --selection_method oracle --n_restarts 10
#
# Output
#   results/exp7/results_{mode}_{config}_{selection_method}.json
...
from utils.dataset_texture import make_environment_texture
...
def load_hparams(mode, config, selection_method):
    path = (f"results/exp7/search_results"
...
def main():
    ...
    # Load data using texture environment builder
    ...
    data = load_environments(
        ...
        device=device,
        make_env_fn=make_environment_texture,
        test_e=TEST_E,
    )
    ...
    os.makedirs("results/exp7", exist_ok=True)
    out = {
        "mode":             args.mode,
        "config":           args.config,
        "e_values":         cfg["e_values"],
    ...
    out_path = (f"results/exp7/results"
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Exp 7 — Two Correlated Spurious Features")
